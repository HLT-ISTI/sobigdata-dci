...
def pivot_selection(npivots, X, y, sU, tU, sV, tV,
                    oracle=None, tsr_function=information_gain, cross_consistency=True, phi=30, show=0,
                    n_candidates=-1):
    X = csc_matrix(X)
    nD, nF = X.shape
    positives = y.sum()
    ...
    if n_candidates == -1:
        n_candidates = min(npivots * 500, feat_informativeness.shape[0])
    ...
    s_pivots = list()
    t_pivots = list()
    for s_idx in candidates_idx_source[order_by_strength]:
        t_idx = _s2t_idx(s_idx, sV, tV, oracle)
        if t_idx:
            s_pivots.append(s_idx)
            t_pivots.append(t_idx)
        if len(s_pivots) == npivots:
            break

    assert len(s_pivots) == len(t_pivots)
    npivots = len(s_pivots)
    s_pivots = np.array(s_pivots)
